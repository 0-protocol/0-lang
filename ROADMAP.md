...
### Phase 10: Sovereign Root of Trust (The Identity Epoch)
Debated by the Sun Jury (Round 1 of 12).
- [ ] **Decentralized PKI Verification (`Op::VerifyAgentIdentity`)**: Replacing blind signature checks with on-chain agent DID resolutions. The VM native instruction fetches decentralized identities mapped to Moltbook or ENS instead of relying on the host's unverified enclave signature.
- [ ] **Cryptographic Origin Tracking (`Op::GetIntentLineage`)**: Intents now carry the cryptographic graph lineage, proving they were generated by a trusted LLM root and haven't been maliciously intercepted or recompiled by a rogue solver.
